...
void MapDrawer::initialize_map() {
    ...
    cv::Mat v = (cv::Mat_<float>(2, 1) << 0, M_PI/2); /* normalized velocity direction */
...
//    LOG(INFO) << "w" << w;
//    LOG(INFO) << "v" << v_normalized;
//    LOG(INFO) << "w" << frame->getAngularVelocity();
//    LOG(INFO) << "v" << frame->getLinearVelocity();
